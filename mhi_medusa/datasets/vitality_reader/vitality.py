...
class CocoDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, key):
        ...
        # print(f"try to get\n\tkey: {key, type(key)}\n\tidx: {idx, type(idx)}\n\timage_id: {image_id,type(image_id)}")

        # get mhi
        file_name = self.coco.loadImgs(image_id)[0]['file_name']
        # print(self.root)
        img, target = super(CocoDetection, self).__getitem__(int(idx))
        # print("get img,target")
        # depth = Image.open(os.path.join(
        #     str(self.root) + "_depth", file_name)).convert('L')
        # print(str(self.root)[:-4] + "_mhi/*_" + file_name[-12:-4] + ".npy")
        mhi_path = glob(str(self.root)[:-4] +
                        "_mhi/*_" + file_name[-12:-4] + ".npy")
        # print(mhi_path)
        assert len(
            mhi_path) == 1, f"more than one (or zero) mhi fit in dir: {len(mhi_path)}"
        mhi = np.load(mhi_path[0])
        ...
        img, depth, target = self._transforms(RGB_mhi, XYZ_mhi, target)
    ...
    def generate_subset_mapping(self, subset_ratio):
        ...
        path = os.path.join(self.subset_index_dir, str(
            self.image_set) + "_" + str(subset_ratio))
        ...
        if os.path.exists(path):
            ...
            selected_ids = np.sort(np.random.choice(
                total_num, subset_num, replace=False))
...
def make_coco_transforms(image_set, image_size):
    print("call make_coco_transforms")
    orig_height = 800
    orig_width = 1333
    orig_random_heights = [480, 512, 544, 576,
    ...
    adj_random_heights = [int(scale * height_scale)
                          for scale in orig_random_heights]
    adj_crop_random_heights = [int(scale * height_scale)
                               for scale in orig_crop_random_heights]
...
def build(image_set, args):
    print("building abnormal fish dataset")
    root = Path(args.path)
    image_size = (args.image_height, args.image_width)
    subset_ratio = args.subset_ratio

    assert root.exists(), f'provided abnormal_fish path {root} does not exist'
    mode = 'instances'
    PATHS = {
        "train": (root / "train_img", root / "annotations" / f'train.json'),
        "val": (root / "val_img", root / "annotations" / f'val.json'),
    }
